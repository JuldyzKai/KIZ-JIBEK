...
<head>
    ...
    <style>
        ...
        .overlay {
            ...
            width: 100%;
            height: 100%;
            background-color: rgba(234, 232, 159, 0.164);
            z-index: -1;
        }
        ...
        .container {
            ...
            margin: 20px auto;
            padding: 20px;
            background-color: rgba(229, 142, 130, 0.545);
            border-radius: 15px;
        ...
        h1 {
            font-size: 3.5rem;
            margin-bottom: 10px;
            text-shadow: 2px 2px 4px rgb(12, 37, 219);
            color: #ebe373;
        }
        ...
        .wheel {
            ...
            position: relative;
            overflow: hidden;
            border: 15px solid #e2d815;
            box-shadow: 0 0 0 15px #3a3a3ac2, 0 0 40px rgba(192, 119, 119, 0.8);
...
<body>
    ...
    <button class="results-btn" id="resultsBtn">НӘТИЖЕ</button>
    <button class="fullscreen-btn" id="fullscreenBtn">эк</button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function toggleFullscreen() {
                if (!document.fullscreenElement) {
                    document.documentElement.requestFullscreen().catch(err => {
                        alert(`Толық экран режимін іске қосу мүмкін емес: ${err.message}`);
                    });
                } else {
            ...
            // Интерактивті тақтаға арналған стильдер
            function adjustForTouchScreen() {
                const isTouchScreen = ('ontouchstart' in window) || (navigator.maxTouchPoints > 0);
                
                if (isTouchScreen) {
                    // UI элементтерін үлкейту
                    document.querySelectorAll('button').forEach(btn => {
                        btn.style.fontSize = '24px';
                        btn.style.padding = '20px 40px';
                    });
                    
                    // Дөңгелектерді үлкейту
                    document.querySelectorAll('.wheel-box').forEach(wheel => {
